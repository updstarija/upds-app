...
const ELibro = () => {
  ...
  const getSessionElibro = async () => {
    const dataTokens = await fetch(
      "https://portal.upds.edu.bo/gapi/request/service/?path=updsnet/access/infoelibro/2"
    );
    const accessTokens = await await dataTokens.json();

    const secret = accessTokens?.find(
      (x: any) => x.Nombre === "ELIBRO_SECRET"
    )?.ValorCadena;
    const channel_id = accessTokens?.find(
      (x: any) => x.Nombre === "ELIBRO_CHANNEL_ID"
    )?.ValorCadena;
    ...
    myHeaders.append(
      "Authorization",
      "Token a70f0600234c22e6caf12e75df3e254f7fd2a1dc"
    );
    ...
    const dataElibro = await fetch(
      "https://auth.elibro.net/auth/sso/",
      requestOptions
    );
