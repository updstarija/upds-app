      ...
      >
        <View className="flex-row justify-center mt-5">
          {sliders.map((_, index) => (
            <View
              key={index}
              className={`h-[5] bg-[#0D1F46] w-3 rounded mx-[1.5] ${
                sliderActual === index ? "w-5 bg-white dark:bg-[#223B82]" : ""
              }`}
            />
          ))}
        </View>
        ...
        <View style={{ marginBottom: 20 }}>
          {sliderActual == sliders.length - 1 ? (
            <View style={{ height: 50 }}>
              ...
              >
                <Text className="text-xl font-bold text-center text-[#223B82] dark:text-white">
                  Comenzar
                </Text>
              </TouchableOpacity>
            </View>
      ...
      <ScrollView contentContainerStyle={{ flex: 1 }}>
        <View className="flex-1  bg-[#223B82] dark:bg-[#040e22] ">
          <FlatList
            ...
            renderItem={({ item }) => <Slide {...item} key={item.text} />}
          />
    ...
    >
      {title && (
        <View>
          ...
          <Texto
            className=" text-white font-bold text-3xl lg:text-6xl text-center uppercase"
            weight="Bold"
          >
            ¡UPDS Tarija!
          </Texto>
      ...
      <View className="flex-column items-center mt-10">
        ...
        />
        <Texto className="text-lg lg:text-2xl text-white dark:text-white text-center">
          {text}
        </Texto>
      </View>
    </View>
