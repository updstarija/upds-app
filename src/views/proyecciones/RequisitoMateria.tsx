...
const alertPrerequisito = () => {
  Alert.alert(
    "Informacion",
    "Pre-requisito es una materia previa que se debe aprobar antes de inscribirse en una materia más avanzada o especializada."
  );
};
const alertPostRerequisito = () => {
  Alert.alert(
    "Informacion",
    "Co-requisito es una materia que puede ser cursada solo después de haber aprobado una materia previa relacionada."
  );
};
...
export const RequisitoMateria: React.FC<Props> = ({ materia }) => {
  ...
  const [materiasHistory, setMateriasHistory] = useState([
    materia.materiaAdmId,
  ]);
  const [materiasNombreHistory, setMateriasNombreHistory] = useState([
    materia.materia,
  ]);
  ...
  const addNewMateriaHistory = (materia: MateriaRequisito) => {
    ...
    const posicionExistente = materiasHistory.findIndex(
      (mat) => mat === materia.id
    );
  ...
  return (
    <View className="">
      <View className="flex-row justify-between items-start mx-4">
        {selectedMateria > 0 ? (
          <TouchableOpacity onPress={prevMateria}>
            <AntDesign
              name="leftcircleo"
              size={20}
        ...
        <View style={{ maxWidth: 300 }}>
          {data.isLoading ? (
            ...
            <Texto
              weight="Bold"
              className="text-center text-xl mb-4 dark:text-white"
            >
              {materiasNombreHistory[selectedMateria]}
            </Texto>
          )}
        </View>

        {selectedMateria < materiasHistory.length - 1 ? (
          <TouchableOpacity onPress={nextMateria}>
            <AntDesign
              name="rightcircleo"
              size={20}
              color={isDarkMode ? "#FFF" : "#000"}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View className="flex-row gap-3 p-4 items-center">
            {data.isLoading ? (
              ...
              <>
                {data.data.data.prerequisito.length === 0 && (
                  <View className="rounded-full p-2 border-[.5px] border-primario">
                ...
                {data.data.data.prerequisito.map((pre) => (
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => addNewMateriaHistory(pre)}
                    key={pre.id}
                  >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View className="flex-row gap-3 p-4 items-center">
            {data.isLoading ? (
              ...
              <>
                {data.data.data.corequisito.length === 0 && (
                  <View className="rounded-full p-2 border-[.5px] border-primario">
                    <Texto className="text-black dark:text-white">
                      SIN CO-REQUISITO
                    </Texto>
                ...
                {data.data.data.corequisito.map((co) => (
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => addNewMateriaHistory(co)}
                    key={co.id}
                  >
                    <View
                      className="bg-primario block rounded-full p-2 border-[.5px]"
                      style={{ maxWidth: 200 }}
                    >
